...
db=<file name> fasta file of the database, default uniprot_sprot.fasta
...
$fasdb="uniprot_sprot.fasta";
if ("@ARGV"=~/db=(\S+)/) { $fasdb=$1;}
my $ecut=0.0001;
...
while (<FAS>){
	chop;
	if ($_=~/^>(\S+).+gene=(Cladocopium\S+)/){ $f2iso{$1}=$2;}
	elsif ($_=~/^>(\S+).+gene=(Mcavernosa\S+)/){ $f2iso{$1}=$2;}
	elsif ($_=~/^>(\S+)/){ $f2iso{$1}=$1;}
# print "$1\t$f2iso{$1}\n";
}

warn "reading uniprot fasta...\n";
